...
export const ClassicTemplate: React.FC<PuzzleTemplateProps> = ({ puzzle, config, fontFamily }) => {
    ...
    const getThemeStyles = () => {
        if (designTheme === 'classic') {
            return {
                pageStyle: {}, // White background default
                contentStyle: { border: showBorders ? '3px double black' : 'none', padding: '0.5in' }, // Border at margin line
        ...
        if (designTheme === 'kids') {
            return {
                pageStyle: { backgroundColor: isColor ? undefined : '#FFF9E6' }, // Cream background FULL BLEED
                contentStyle: { border: showBorders ? '6px solid #FFB347' : 'none', borderRadius: '20px', padding: '0.25in' }, // Orange border at margin line
                headerTitle: { fontFamily: '"Comic Sans MS", cursive', color: '#FF4500', textShadow: '2px 2px 0px #FFD700', letterSpacing: '0.05em' },
                gridBorder: { border: showBorders ? '4px dashed #77DD77' : 'none', borderRadius: '15px', backgroundColor: 'rgba(255,255,255,0.8)' }, // Pastel Green
        ...
        return { pageStyle: {}, contentStyle: {}, headerTitle: {}, gridBorder: {}, wordListTitle: {} };
    };

    const themeStyles = getThemeStyles();

    // Container background: Transparent if image exists so we can see it
    const pageBackgroundStyle = isColor
    ...
    const estimatedFooterHeight = 3.0 + (words.length > 20 ? 0.5 : 0);
    ...
    const getTextCardStyle = (addMarginBottom: boolean = true): React.CSSProperties => {
        if (!hasBackground && designTheme !== 'modern') {
            return { marginBottom: addMarginBottom ? '1rem' : '0' };
        }
        return {
    ...
    const gridContainerStyle: React.CSSProperties = {
        ...
        ...(config.maskShape !== 'SQUARE' && config.maskShape !== 'CIRCLE' ? { border: 'none' } : {}),
    ...
    return (
        ...
        >
            {backgroundImage && (
                <div className="absolute inset-0 w-full h-full z-0 pointer-events-none">
                    <img
                        ...
                        style={{
                            opacity: backgroundStyle === 'bw' ? 1.0 : 0.95,
            ...
            >
                {/* Header */}
                <div className="w-full flex-shrink-0 transition-all" style={getTextCardStyle(true)}>
                    <div
                        className={`w-full ${!hasBackground && designTheme !== 'classic' ? 'border-b-2 pb-2' : ''}`}
                        style={{ borderColor: headerTextColor, color: headerTextColor }}
                    >
                        <h1 className="text-4xl font-bold text-center uppercase tracking-wider mb-2"
                            style={{
                                fontFamily: config.fontType === 'FUN' ? fontFamily : 'Inter',
                ...
                <div className="flex-grow flex items-center justify-center w-full mb-4 relative min-h-0">
                    <div
                        className={`transition-all duration-300 ${config.maskShape === 'SQUARE' && !backgroundImage && designTheme !== 'classic' ? 'border-2' : ''}`}
                ...
                {/* Word List */}
                <div className="w-full mt-auto flex-shrink-0 transition-all" style={getTextCardStyle(true)}>
                    <h3
                        ...
                        style={{
                            ...
                            borderColor: 'black',
                            fontFamily: config.fontType === 'FUN' ? fontFamily : 'inherit',
                            ...themeStyles.wordListTitle
                        }}
                    >
                        Palabras a encontrar:
                    </h3>
                    <div
                        className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-xs sm:text-sm font-medium w-full px-4"
                        style={{
                            color: isColor ? themeData.textColor : 'black',
                            fontFamily: config.fontType === 'FUN' ? fontFamily : 'inherit'
                        }}
                ...
                {/* Footer */}
                <div className="w-full flex-shrink-0 transition-all" style={getTextCardStyle(false)}>

...
import { Difficulty, GeneratedPuzzle, PuzzleTheme, AppSettings, SavedPuzzleRecord, PuzzleConfig, AIProvider, ShapeType, FontType, AISettings, ArtTemplate, PuzzleMargins, BookStack, ImageFilters, CustomTemplate } from './types';
...
export default function App() {
    ...
    const [showBrainConsole, setShowBrainConsole] = useState(false); // New Brain Console State
    ...
    return (
        <div className="flex h-screen text-slate-200 overflow-hidden font-sans selection:bg-indigo-500/30" style={{ background: 'linear-gradient(135deg, #0a0e27 0%, #1a1535 50%, #0f051d 100%)' }}>
                            ...
                            activeSidebarTab === 'library' ? (
                                ...
                                <PropertyPanel
                                    ...
                                    onUpdateConfig={(updates) => {
                                        ...
                                        // Independent typography per element
                                        if (updates.fontFamilyHeader !== undefined) setFontFamilyHeader(updates.fontFamilyHeader);
                                        if (updates.fontFamilyGrid !== undefined) setFontFamilyGrid(updates.fontFamilyGrid);
                                        if (updates.fontFamilyWordList !== undefined) setFontFamilyWordList(updates.fontFamilyWordList);
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="space-y-3 pt-4 border-t border-white/5">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <button
                                    onClick={() => setShowBrainConsole(true)}
                                    className="w-full relative overflow-hidden group rounded-xl p-[1px]"
                                >
            ...
            {
                showBrainConsole && (
                    <div className="fixed inset-0 z-[60] bg-cosmic-950 flex flex-col animate-in fade-in duration-300 overflow-auto">
                        <div className="p-4 border-b border-glass-border flex justify-between items-center bg-cosmic-900 sticky top-0 z-10">
                            <h2 className="text-xl font-bold flex items-center gap-2 text-white">
                                <BrainCircuit className="w-6 h-6 text-purple-500" /> The Brain Console
                            </h2>
                            <button
                                onClick={() => setShowBrainConsole(false)}
                        ...
                        <div className="flex-1 p-8">
                            <BrainConsole
                                ...
                                onApplyTemplate={(t) => {
                                    setDesignTheme(t.designTheme);
                                    setShowBorders(t.showBorders);
                                    if (t.themeData) setThemeData(t.themeData);
                                    if (t.margins) setMargins(t.margins);
                                    setFontType(t.fontType);
                                    setShowBrainConsole(false);
                                    showToast(`Plantilla "${t.name}" aplicada`, 'success');
                                }}
                                puzzleLibrary={libraryPuzzles}
                                onLoadPuzzle={(p) => {
                                    handleLoadPuzzle(p);
                                    setShowBrainConsole(false);
                                }}

...
export default function App() {
    ...
    const capturePuzzleSnapshot = async () => {
        ...
        try {
            ...
            // Actually, we want the EXACT look, so we capture what is there.
            // We use a high scale to look sharp on the studio canvas.
            const canvas = await (window as any).html2canvas(element, {
                scale: 2,
                useCORS: true,
                backgroundColor: null // Transparent logic handled by CSS or existing background
            });
                    ...
                    <div className="space-y-3 pt-4 border-t border-white/5">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <button
                                ...
                                className="w-full relative overflow-hidden group rounded-xl p-[1px]"
                            >
                                <div className="absolute inset-0 bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 opacity-70 group-hover:opacity-100 transition-opacity duration-500"></div>
                                <div className="relative bg-cosmic-950 rounded-xl p-3 flex flex-col items-center justify-center gap-2 group-hover:bg-cosmic-900 transition-colors h-20">
                                    <Wand2 className="w-5 h-5 text-emerald-400 group-hover:text-white transition-colors" />
                                    <span className="font-bold text-[10px] text-slate-200 group-hover:text-white transition-colors text-center">Transformar Sopa</span>
                                </div>
                            </button>
                        </div>

                        <p className="text-[10px] text-slate-500 text-center">
                            Genera diseños o transforma tu sopa actual con IA.
                        </p>
                    </div>
